...
class PEG(nn.Module):
    def __init__(self, dim, k=3) -> None:
        ...
        trunc_normal_(self.pos.weight.data, std=0.02)
        # self.pos.weight.data.zero_()
        self.pos.bias.data.zero_()
        
        # fan_out = self.pos.kernel_size[0] * self.pos.kernel_size[1] * self.pos.out_channels
        # fan_out //= self.pos.groups
        # self.pos.weight.data.normal_(0, math.sqrt(2.0 / fan_out))
        # if self.pos.bias is not None:
        #     self.pos.bias.data.zero_()
        
    def forward(self, x, H, W, cls_pos, other_pos):
        B, N, C = x.shape
        cls_tokens, feat_tokens = x[:, cls_pos], x[:, other_pos]
        feat_tokens = feat_tokens.transpose(1,2).view(B, C, H, W)
        feat_tokens = self.pos(feat_tokens) + feat_tokens # B, C, H, W
        output = x.detach().clone()
        output[:, cls_pos] = cls_tokens
        output[:, other_pos] = feat_tokens.flatten(2).transpose(1, 2) # B, N, C
        return output
